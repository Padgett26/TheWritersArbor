...
function destroySession() {
	...
	if (ini_get ( "session.use_cookies" )) {
		...
		setcookie ( session_name (), '', time () - 1209600, $params ["path"], $params ["domain"], $params ["secure"], $params ["httponly"] );
	}
	session_destroy ();
}
...
if (filter_input ( INPUT_POST, 'login', FILTER_SANITIZE_NUMBER_INT ) == "1") {
	...
	if ($login2row) {
		if ($login2row ['id']) {
			$x = $login2row ['id'];
			$_SESSION ['myId'] = $x;
			setcookie ( "staySignedIn", $x, $time + 1209600, "/", $domain, 0 ); // set for 14 days
		} else {
...
$groupId = 0;
if ($page == "group" && filter_input ( INPUT_GET, 'groupId', FILTER_SANITIZE_NUMBER_INT ) >= 1) {
