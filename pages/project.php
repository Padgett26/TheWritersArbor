...
if ($myId >= 1) {
	...
	if (filter_input ( INPUT_POST, 'writingUp', FILTER_SANITIZE_NUMBER_INT ) >= 1) {
		...
		$writing = htmlspecialchars ( filter_input ( INPUT_POST, 'writing', FILTER_UNSAFE_RAW ) );
		$delPic = (filter_input ( INPUT_POST, 'delPic', FILTER_SANITIZE_NUMBER_INT ) == 1) ? 1 : 0;
		if ($id == 0) {
			$wUpNew = $db->prepare ( "INSERT INTO writings VALUES(NULL, ?, ?, ?, ?, ?, ?, '0', '0', '0')" );
			$wUpNew->execute ( array (
					$myId,
					$p,
					$time,
					$time,
			...
			$getNew = $db->prepare ( "SELECT id FROM writings WHERE userId = ? AND projectId = ? AND submissionDate = ? ORDER BY id DESC LIMIT 1" );
			...
			$setP = $db->prepare ( "INSERT INTO images VALUES(NULL, ?, ?, '0', '0')" );
			$setP->execute ( array (
					$id,
					'x.png'
			) );

			// Send email notification to member of the group, who want them, about a new submission
			...
			if ($n1R) {
				$g = $n1R ['groupId'];
				$n2 = $db->prepare ( "SELECT userId FROM groupMembership WHERE groupId = ?" );
				$n2->execute ( array (
						$g
				) );
				while ( $n2R = $n2->fetch () ) {
					if ($n2R) {
						$u = $n2R ['userId'];
						$n3 = $db->prepare ( "SELECT name, email, groupNotification FROM users WHERE id = ?" );
						$n3->execute ( array (
								$u
						) );
						$n3R = $n3->fetch ();
			...
			$update = $db->prepare ( "UPDATE writings SET lastEditDate = ?, title = ?, writing = ? WHERE id = ?" );
		...
		if (isset ( $_FILES ['image'] ['tmp_name'] ) && $_FILES ['image'] ['size'] >= 1000) {
			...
			if ($width != null && $height != null) {
				...
				$p1stmt = $db->prepare ( "UPDATE images SET imageName = ? WHERE writingsId = ?" );
		...
		if ($delPic == 1) {
			$pstmt = $db->prepare ( "UPDATE images SET imageName = ? WHERE writingsId = ?" );
			$pstmt->execute ( array (
					'x.png',
					$id
			) );
	...
	if (filter_input ( INPUT_POST, 'noteUp', FILTER_SANITIZE_NUMBER_INT ) >= 1) {
		...
		$note = htmlentities ( filter_input ( INPUT_POST, 'note', FILTER_SANITIZE_STRING ), ENT_QUOTES );
		$nUp = $db->prepare ( "INSERT INTO notes VALUES(NULL, ?, ?, ?, ?, '0', '0')" );
		$nUp->execute ( array (
				$myId,
				$w,
				$note,
		...
		// Send email notification to member of the group, who want them, about a new submission
		$wn = $db->prepare ( "SELECT userId, title, projectId FROM writings WHERE id = ?" );
		$wn->execute ( array (
				$w
		) );
		$wnR = $wn->fetch ();
		if ($wnR) {
			...
			$n3 = $db->prepare ( "SELECT name, email, writingNotification FROM users WHERE id = ?" );
			$n3->execute ( array (
					$writingId
			) );
			$n3R = $n3->fetch ();
	...
	if ($p >= 1) {
		...
		if ($getPR) {
			...
			$description = nl2br ( html_entity_decode ( $getPR ['description'], ENT_QUOTES ) );
			...
			$in = "x.png";
			$getW = $db->prepare ( "SELECT * FROM writings WHERE userId = ? AND projectId = ? LIMIT 1" );
			$getW->execute ( array (
					$myId,
					$p
			) );
			...
			if ($getWR) {
				...
				$getI = $db->prepare ( "SELECT * FROM images WHERE writingsId = ? LIMIT 1" );
				$getI->execute ( array (
						$wId
				) );
				$getIR = $getI->fetch ();
			...
			if ($wId != 0) {
				...
				echo "</form>\n";
				$notes = $db->prepare ( "SELECT * FROM notes WHERE writingsId = ? ORDER BY noteDate DESC" );
				$notes->execute ( array (
						$wId
				) );
				while ( $notesR = $notes->fetch () ) {
					if ($notesR) {
						$u = $notesR ['userId'];
						$n = nl2br ( html_entity_decode ( $notesR ['note'], ENT_QUOTES ) );
						$d = $notesR ['noteDate'];
						...
						echo "<div style='padding:20px 0px;'>" . showDate ( $d ) . " - $un<br>$n</div>\n";
					}
				}
			}
			echo "</div></div></div>\n";
			...
			echo "<div style='padding:30px;'>\n";
			$in = "x.png";
			$getW = $db->prepare ( "SELECT * FROM writings WHERE projectId = ? ORDER BY RAND()" );
			$getW->execute ( array (
					$p
			) );
			while ( $getWR = $getW->fetch () ) {
				if ($getWR) {
					...
					$wTitle = html_entity_decode ( $getWR ['title'], ENT_QUOTES );
					$wWriting = nl2br ( html_entity_decode ( $getWR ['writing'], ENT_QUOTES ) );
					preg_match_all ( "/<hebrew>([a-z\s]+)<\/hebrew>/", $wWriting, $match, PREG_SET_ORDER );
					...
					list ( $width, $height ) = (getimagesize ( $in ) != null) ? getimagesize ( $in ) : null;
					...
					echo "<div style='font-size:.75em; padding-top:10px;'>Submission Date: " . showDate ( $wSubmissionDate ) . "</div>\n";
					echo "<div style='font-size:.75em; padding-bottom:10px;'>Last Edit Date: " . showDate ( $wLastEditDate ) . "</div>\n";
					...
					while ( $notesR = $notes->fetch () ) {
						if ($notesR) {
							...
							echo "<div style='padding:20px 0px;'><span style='font-weight:bold;'>" . showDate ( $d ) . " - $un</span><br>$n</div>\n";
						}
					}
					echo "</div></div>\n";
				}
