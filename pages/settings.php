...
if ($myId >= 1) {
	if (filter_input(INPUT_POST, 'block', FILTER_SANITIZE_NUMBER_INT)) {
		...
		$bl = $db->prepare("UPDATE groupMembership SET blocked = ? WHERE userId = ? AND groupId = ?");
		$bl->execute(array(
				'1',
				$memId,
				$grId
	...
	if (filter_input(INPUT_POST, 'newOwner', FILTER_SANITIZE_NUMBER_INT) >= 1) {
		$newOwner = filter_input(INPUT_POST, 'newOwner', FILTER_SANITIZE_NUMBER_INT);
		$newOwnerGroup = filter_input(INPUT_POST, 'newOwnerGroup', FILTER_SANITIZE_NUMBER_INT);
		$oUp = $db->prepare("UPDATE groups SET creatorId = ? WHERE id = ?");
		$oUp->execute(array(
				$newOwner,
				$newOwnerGroup
		));
		$recordChange = $db->prepare("INSERT INTO groupOwnershipChanges VALUES(NULL,?,?,?,?,'0')");
		$recordChange->execute(array(
				$myId,
				$newOwner,
				$time,
				$newOwnerGroup
	...
	if (filter_input(INPUT_POST, 'invites', FILTER_SANITIZE_NUMBER_INT) == 1) {
		...
		for ($i = 1; $i <= 5; ++ $i) {
			${"name" . $i} = filter_input(INPUT_POST, "name$i", FILTER_SANITIZE_STRING);
		}
		for ($j = 1; $j <= 5; ++ $j) {
			${"email" . $j} = filter_input(INPUT_POST, "email$j", FILTER_SANITIZE_EMAIL);
		}
		for ($k = 1; $k <= 5; ++ $k) {
			if (${"name" . $k} != "" && ${"name" . $k} != " " && ${"email" . $k} != "" && ${"email" . $k} != " ") {
				$iCheck = $db->prepare("SELECT COUNT(*) FROM invites WHERE email = ? AND inviteGroup = ?");
				...
				if ($iCheckR && $iCheckR[0] == 0) {
					$iUp = $db->prepare("INSERT INTO invites VALUES(NULL, ?, ?, ?, ?, '0', '0')");
					$iUp->execute(array(
							${"name" . $k},
							${"email" . $k},
							$gid,
				...
				$iGetId = $db->prepare("SELECT id FROM invites WHERE email = ? AND inviteGroup = ? LIMIT 1");
				...
				if ($iGetIdR) {
					$u = $iGetIdR['id'];
					$emailBody = nl2br(filter_input(INPUT_POST, 'emailBody', FILTER_SANITIZE_STRING));
					$emailBody .= "To join this group, please follow this link: https://thewritersarbor.com/index.php?page=invites&u=$u Thank you";
					sendEmail("$MYNAME has invited you to a group on The Writers Arbor", $emailBody, ${"email" . $k}, ${"name" . $k});
				}
			}
		}
	}
	if (filter_input(INPUT_POST, 'personalUp', FILTER_SANITIZE_NUMBER_INT) == 1) {
		...
		$e = (filter_input(INPUT_POST, 'email', FILTER_VALIDATE_EMAIL)) ? filter_input(INPUT_POST, 'email', FILTER_SANITIZE_EMAIL) : 'x';
		$pwd1 = filter_input(INPUT_POST, 'pwd1', FILTER_SANITIZE_STRING);
		$pwd2 = filter_input(INPUT_POST, 'pwd2', FILTER_SANITIZE_STRING);
		$groupNotification = (filter_input(INPUT_POST, 'groupNotification', FILTER_SANITIZE_NUMBER_INT) == 1) ? 1 : 0;
		$writingNotification = (filter_input(INPUT_POST, 'writingNotification', FILTER_SANITIZE_NUMBER_INT) == 1) ? 1 : 0;
		if ($e == 'x') {
			...
			$updateP = $db->prepare("UPDATE users SET name = ?, email = ?, groupNotification = ?, writingNotification = ? WHERE id = ?");
			$updateP->execute(array(
					$n,
					$e,
					$groupNotification,
					$writingNotification,
	...
	if (filter_input(INPUT_POST, 'groupUp', FILTER_SANITIZE_NUMBER_INT)) {
		...
		$gTitle = htmlentities(filter_input(INPUT_POST, 'title', FILTER_SANITIZE_STRING), ENT_QUOTES);
		$gDesc = htmlentities(filter_input(INPUT_POST, 'desc', FILTER_SANITIZE_STRING), ENT_QUOTES);
		$gSuspend = (filter_input(INPUT_POST, 'suspend', FILTER_SANITIZE_NUMBER_INT) == 1) ? 1 : 0;
		$gUnsuspend = (filter_input(INPUT_POST, 'unsuspend', FILTER_SANITIZE_NUMBER_INT) == 1) ? 1 : 0;
		$gClose = (filter_input(INPUT_POST, 'close', FILTER_SANITIZE_NUMBER_INT) == 1) ? 1 : 0;
		$gUnclose = (filter_input(INPUT_POST, 'unclose', FILTER_SANITIZE_NUMBER_INT) == 1) ? 1 : 0;
		$gDelLogo = (filter_input(INPUT_POST, 'delLogo', FILTER_SANITIZE_NUMBER_INT) == 1) ? 1 : 0;
		$openGroup = filter_input(INPUT_POST, 'openGroup', FILTER_SANITIZE_NUMBER_INT);
		$sus = ($gSuspend == 1) ? $time : 0;
		$sus = ($gUnsuspend == 1) ? 0 : $sus;
		$close = ($gClose == 1) ? $time : 0;
		$close = ($gUnclose == 1) ? 0 : $close;
		$updateG = $db->prepare("UPDATE groups SET suspendDate = ?, closedDate = ?, title = ?, description = ?, openGroup = ? WHERE id = ?");
		$updateG->execute(array(
				$sus,
				$close,
				$gTitle,
				$gDesc,
		...
		if ($gDelLogo == 1) {
			...
			if (isset($_FILES['image']['tmp_name']) && $_FILES['image']['size'] >= 1000) {
				...
				if ($width != null && $height != null) {
					...
					$p1stmt = $db->prepare("UPDATE groups SET logoPic = ? WHERE id = ?");
		...
		foreach ($_POST as $key => $val) {
			if (preg_match("/^cat([1-9][0-9]*)$/", $key, $match)) {
				if ($val == 1) {
					$catId = $match[1];
					$gm = $db->prepare("INSERT INTO groupCategories VALUES(NULL,?,?,?)");
					$gm->execute(array(
							$gId,
							$catId,
							'0'
		...
		$catNew = (filter_input(INPUT_POST, 'catNew', FILTER_SANITIZE_NUMBER_INT) == 1) ? 1 : 0;
		if ($catNew == 1) {
			$catNewName = filter_input(INPUT_POST, 'catNewName', FILTER_SANITIZE_STRING);
			if ($catNewName != "" && $catNewName != " ") {
				$cnn = htmlentities($catNewName, ENT_QUOTES);
				$cCheck = $db->prepare("SELECT COUNT(*),id FROM categories WHERE catName = ?");
				...
				if ($cc == 0) {
					$cUp = $db->prepare("INSERT INTO categories VALUES(NULL,?,?,?)");
					$cUp->execute(array(
							$cnn,
							'0',
							'0'
					));
					$cGet = $db->prepare("SELECT id FROM categories WHERE catName = ?");
					$cGet->execute(array(
							$cnn
					));
					$cGetR = $cGet->fetch();
				...
				$gcUpCheck = $db->prepare("SELECT COUNT(*) FROM groupCategories WHERE catId = ? AND groupId = ?");
				...
				if ($gcUCR) {
					$gcCount = $gcUCR[0];
					if ($gcCount == 0) {
						$gcUp = $db->prepare("INSERT INTO groupCategories VALUES(NULL,?,?,?)");
						$gcUp->execute(array(
								$gId,
								$cId,
								'0'
	...
	if (filter_input(INPUT_POST, 'projectUp', FILTER_SANITIZE_NUMBER_INT)) {
		$projectId = filter_input(INPUT_POST, 'projectUp', FILTER_SANITIZE_NUMBER_INT);
		$projectTitle = htmlentities(filter_input(INPUT_POST, 'title', FILTER_SANITIZE_STRING), ENT_QUOTES);
		$projectDesc = htmlentities(filter_input(INPUT_POST, 'desc', FILTER_SANITIZE_STRING), ENT_QUOTES);
		...
		$updateP = $db->prepare("UPDATE projects SET openDate = ?, closeDate = ?, title = ?, description = ? WHERE id = ?");
		$updateP->execute(array(
				$open,
				$close,
				$projectTitle,
	...
	if (filter_input(INPUT_POST, 'groupUp', FILTER_SANITIZE_STRING) == 'new') {
		...
		$gDesc = htmlentities(filter_input(INPUT_POST, 'desc', FILTER_SANITIZE_STRING), ENT_QUOTES);
		$openGroup = filter_input(INPUT_POST, 'openGroup', FILTER_SANITIZE_NUMBER_INT);
		$createG = $db->prepare("INSERT INTO groups VALUES(NULL,?,?,'0','0',?,?,'x.png','x.png',?,'0')");
		...
		$getG = $db->prepare("SELECT id FROM groups WHERE creatorId = ? AND createDate = ?");
		$getG->execute(array(
				$myId,
				$time
		));
	...
	if (filter_input(INPUT_POST, 'projectUp', FILTER_SANITIZE_STRING) == 'new') {
		...
		$open = mktime(0, 0, 0, $po[1], $po[2], $po[0]);
		$close = mktime(0, 0, 0, $pc[1], $pc[2], $pc[0]);
		$updateP = $db->prepare("INSERT INTO projects VALUES(NULL,?,?,?,?,?,'1','0','0')");
		$updateP->execute(array(
				$open,
				$close,
				$groupId,
	...
	$getUser = $db->prepare("SELECT name, email, groupNotification, writingNotification FROM users WHERE id = ?");
	...
	$getG = $db->prepare("SELECT * FROM groups WHERE creatorId = ? ORDER BY closedDate");
	$getG->execute(array(
			$myId
	));
	while ($getGR = $getG->fetch()) {
		if ($getGR) {
			...
			$get = $db->prepare("SELECT userId FROM groupMembership WHERE groupId = ? AND blocked = ?");
			$get->execute(array(
					$id,
					'0'
			));
			while ($getR = $get->fetch()) {
				if ($getR) {
					...
					$getNR = $getN->fetch();
					if ($getNR) {
						$userName = html_entity_decode($getNR['name'], ENT_QUOTES);
			...
			foreach ($CATEGORIES as $k => $v) {
				if ($v != 0) {
					...
					$getGC = $db->prepare("SELECT COUNT(*) FROM groupCategories WHERE groupId = ? AND catId = ?");
					$getGC->execute(array(
							$id,
							$k
					));
			...
			if ($suspended == 0) {
				...
				echo showDate($suspended) . " Unsuspend - <input type='checkbox' name='unsuspend' value='1'>\n";
			}
			echo "<br>\n";
			echo "Closed date: \n";
			if ($closed == 0) {
				...
				echo showDate($closed) . " Unclose - <input type='checkbox' name='unclose' value='1'>\n";
			}
			echo "<br>\n";
			...
			$gi = $db->prepare("SELECT * FROM invites WHERE inviteGroup = ? ORDER BY inviteDate");
			$gi->execute(array(
					$id
			));
			while ($giR = $gi->fetch()) {
			...
			$am1 = $db->prepare("SELECT userId FROM groupMembership WHERE groupId = ? AND blocked = ?");
			$am1->execute(array(
					$id,
					'0'
			));
			...
			$am2 = $db->prepare("SELECT id, userId FROM groupMembership WHERE groupId = ? AND blocked = ?");
			$am2->execute(array(
					$id,
					'1'
			));
			...
			echo "<input type='date' name='open' value='" . showDate($time) . "'><br>\n";
			echo "<label for='close'>Close Date</label>\n";
			echo "<input type='date' name='close' value='" . showDate($time + 1209600) . "'><br>\n";
			...
			echo "<div style='text-align:center; margin:20px 0px;''><hr style='width:50%; color:#11bd4a;'></div>\n";
			$getP = $db->prepare("SELECT * FROM projects WHERE groupId = ? ORDER BY openDate DESC");
			$getP->execute(array(
					$id
			));
			while ($getPR = $getP->fetch()) {
				if ($getPR) {
					...
					$pDesc = html_entity_decode($getPR['description'], ENT_QUOTES);
					...
					echo "<label for='open'>Open Date</label>\n";
					echo "<input type='date' name='open' value='" . showDate($pOpenDate) . "'><br>\n";
					echo "<label for='close'>Close Date</label>\n";
					echo "<input type='date' name='close' value='" . showDate($pCloseDate) . "'><br>\n";
